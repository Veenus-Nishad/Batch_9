...
@Composable
fun EachProductDetailUi(viewModel: ViewModel = hiltViewModel(), navController: NavController, productID:String) {

    val productIdState=viewModel.getProductByIdState.collectAsState()
    ...
    when{
        productIdState.value.isLoading->{
            CircularProgressIndicator()
        }
        productIdState.value.error!!.isNotEmpty()->{
            Toast.makeText(localContext,productIdState.value.error, Toast.LENGTH_SHORT).show()
        }
        productIdState.value.data !=null ->{
            val product=productIdState.value.data!!
            ...
            Column(
                ...
                Text(
                    product.name, modifier = Modifier.fillMaxWidth(), style = TextStyle(
                ...
                Spacer(Modifier.height(4.dp))
                Text(
                    text = "Rs. ${product.price}", modifier = Modifier.fillMaxWidth(), style = TextStyle(
